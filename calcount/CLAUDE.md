...
| `calories.html` | The free public page, 2026-09-15: "How many calories are in your order?" Tap foods into an order, change servings, share it. No account, stores nothing. A link can open it filtered, `calories.html?brand=Mang Inasal` or `?cat=drinks` or `?q=chickenjoy`, which is what a post or a reel links to. Tested in the browser at 375px, and at double text size, where the open order is the page's one scrolling area so its buttons can always be reached. It is the cheapest test of demand: if people use and share this, the app earns the months. |
| `_test.html` | 117 checks, all passing, 2026-09-15. It sets the browser's own CALCOUNT data aside and puts it back exactly. |
