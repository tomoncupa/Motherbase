...
| A todo with no start sorts by when it was ticked, or when it was written if it is still open. A todo moved to another day stays on the days it was moved from, as > at the bottom, not counted; it is to be done only on its new day. Both rules are `shared/journal.js`, the same in STATUS. | Tom, 2026-09-14 |
