...
| Snap: when a scroll or drag stops within a fifth of the screen of an edge, the view glides onto it. The edge is a day in Day to Weeks and in the notebook, a week in Month, a month in Quarter, a quarter in Year. | Tom asked, Claude chose the edges, 2026-09-13 |
...
| Changing the view never changes the date in focus. One focused date, moved only by his own scrolling, the arrows, TODAY and opening a box; changing the view, the order or the notebook puts it back at the top. At the far end of the timeline the focus is the last day, so Month on the last four weeks is focused on today. Mouse 4 and 5 no longer zoom toward the pointer. | Tom, 2026-09-14 |
...
| A prettier screen, four changes Tom picked from a mockup. Filters live in a panel under a filter icon beside ADD, with a dot on the icon while any kind is hidden; the Filter bar setting is gone, and "Newest first" and "Newest day first" are renamed "Latest first" (lines in a day) and "Newest day at the top". No grid lines: a line only where a month begins, and in Month, Quarter and Year every other week group sits on a soft surface. Today has an accent bar down its left edge; dates are the size of his line in a softer colour (supersedes dates at `--f-5`); column headings are small plain type. Side blocks show a bare count, and the header names the days on screen rather than repeating the month. SETTINGS is a gear icon so the header fits one row. | Tom asked "is there no better prettier way", picked all four, 2026-09-14 |
...
| A bullet is drawn by `shared/journal.js`, exactly as STATUS draws it: the mark, the words, then the time after them ("7:00pm - 7:31pm" for a todo, no "by"), a done todo's box filled and its words struck through, a cancelled one faded and shown but not counted, a todo on a past day as >. Time order is the start. An open todo with a due date sits on its due day. A todo's menu has Move to tomorrow and Cancel it. Ticking is Journal.tick, so a repeat does a round. Supersedes the time-first column, the × for a done todo, and cancelled lines showing nowhere. | Tom, 2026-09-14: *"I don't want differences"*, STATUS wins |
| A todo with no start sorts by when it was ticked, or when it was written if it is still open. A todo moved or rescheduled to a later day stays as > at the bottom, not counted, on the day it sat on and the day it was moved from, and not on the days it was carried through or skips; it is to be done only on its new day. Both rules are `shared/journal.js`, the same in STATUS. | Tom, 2026-09-14 |
