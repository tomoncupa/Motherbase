...
| `cprog` from FORGE | **forge** writes `f-<id>` (`src: 'forge'`), republished on every change, and a client's `c-<pid>` on SEND | the same | the flattened program: a day per week per day, each with `wk` and its plain `day` name; beside each exercise `n`, `rir`, `rest` in seconds, `note` and `catName`. COACH shows an `f-` row and never edits it |
...
| `fex` | **forge** | `prog\|slot id` | `{day, name, cat, ord, n, r, kg, rir, rest, step, note, wk}` — one exercise in one day. `n` `r` `kg` `rir` are week one's; `wk[w]` holds only what week `w` changes, and anything it does not say is the week before's, plus `step` kg on the load |
