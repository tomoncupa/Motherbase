...
checkin/index.html CHECK IN, physique check-ins for Tom and his clients: a front
                   photo and any pose added, weight (STATUS's row) and a few answers, two check-ins
...
| `_smoke.html` | 228 checks over all of the above. Run it after touching any of them. |
...
| `cphoto` | **checkin** | a photo field's id (`front`, or a pose added in Settings), dated | `{img, w, h}` — a JPEG shrunk to 1280px on its long edge. Big, so it lives in IndexedDB |
...
3. Run `_review.html` at the root. It opens every app in turn, checks each one
   ...
   working on the foundation and want the 228 without the apps.
   It must say 228 of 228, or more once you add checks.
...
| `shared/` | The foundation, passing 228 checks on 2026-09-14 (214 of them also at phone width). Every app loads it. |
