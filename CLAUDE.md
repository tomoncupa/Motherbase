...
| `portion/` | Built 2026-09-05 and tested in the browser. Paste or type a nutrition label; it says how much of it hits 50g of protein or any other number, what a weight of it comes to, and what that costs. Saves the answers into the food library as ordinary servings, so STATUS logs them in one tap. Makes the cooked twin of a raw label as its own entry. Ranks the whole library against whatever amount is on screen, which is the comparison. Photographs its own card. Reads Sodium, or converts Salt where a label prints that instead. Kept out of the tester build by `tools/build-client.py`. |
