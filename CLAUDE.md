...
| `cphoto` | **checkin** | a photo field's id (`front`, `side`, `back` or one added), dated | `{img, w, h}` — a JPEG shrunk to 1280px on its long edge. Big, so it lives in IndexedDB |
